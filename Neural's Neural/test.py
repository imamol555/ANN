...
'''
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(predicted_output, tf.cast(y,tf.float32)))
#print('cross-entropy-->',tf.nn.softmax_cross_entropy_with_logits(predicted_output, tf.cast(y, tf.float32)) )
#print("cost is",cost)
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)

# Evaluate model
correct_prediction = tf.equal(tf.cast(tf.argmax(predicted_output,1),tf.float32), tf.cast(y,tf.float32))
...
for i in range(2000):
    ...
    if i%10 == 0:
        train_accuracy = accuracy.eval(feed_dict={
            x: x_list, y: y_list, keep_prob: 1.0})
        print("step %d, training accuracy %g"%(i, train_accuracy))

    train_step.run(feed_dict={x: x_list, y: y_list, keep_prob: 0.5})
